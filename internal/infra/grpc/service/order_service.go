...
func (s *OrderService) ListOrders(ctx context.Context, in *pb.Blank) (*pb.ListOrderOutput, error) {
	...
	output := &pb.ListOrderOutput{
		Orders: make([]*pb.CreateOrderResponse, 0),
	}

	for _, order := range orders {
		output.Orders = append(output.Orders, &pb.CreateOrderResponse{
			Id:         order.ID,
